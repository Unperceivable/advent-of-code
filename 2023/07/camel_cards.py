...
class CamelCardHand():
    ...
    def better_card(self, other_cards):
        for my_card, their_card in zip(self.cards, other_cards):    
            if card_strength[my_card] > card_strength[their_card]:
                return True
            elif card_strength[my_card] == card_strength[their_card]:
                pass
            else:
                return False
...
def camel_cards(camel_cards: "list[str]"):
    ...
    for hand in camel_cards:
        ...
        current_hand = CamelCardHand(cards, bet)
    ...
    for rank, hand in enumerate(hand_ranking, start=1):
        ...
        # print(f"{hand} {rank} {total_winings}")
...
if __name__ == "__main__":
    ...
    with open(camel_cards_input_path) as camel_cards_file:
        ...
        print(f"Total winnings: {camel_cards(camel_cards_input)}")